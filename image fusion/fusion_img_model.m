function [imgf,Y1,Y2] = fusion_img_model(img1,img2,d,diotf,lambda,alpha,fdeta,gammaq,gammav,gammac,maxiter,tol,flag)
s1=double(img1)/255;
s2=double(img2)/255;
[hei,wid]=size(s1);
...
[Y1,X1,~]=solvex(d,diotf,s1_h,gammaq,gammav,gammac,lambda,alpha,fdeta,maxiter,tol);
[Y2,X2,~]=solvex(d,diotf,s2_h,gammaq,gammav,gammac,lambda,alpha,fdeta,maxiter,tol);
A1=sum(abs(X1),3);
A2=sum(abs(X2),3);
if flag == 1  
    r=9;  
